...
2. 只将 flatten + MLP 头改成 attention pooling head。
3. 用于验证低通道问题是否主要来自 head 设计。

用法示例:
    python conformer_lowchannel_b1_poolhead.py --channel_config c3c4 --classes 1,2 --subject 1
"""
...
class ConformerB1(nn.Sequential):
    def __init__(self, emb_size=40, depth=6, n_classes=4, n_channels=22, window_size=8, **kwargs):
        super().__init__(
            ...
            AttentionPoolingHead(emb_size, n_classes),
        )
...
class ExP:
    ...
    def train(self):
        ...
        test_data = Variable(test_data.float().to(self.device))
        test_label = Variable(test_label.long().to(self.device))
...
def parse_args():
    parser = argparse.ArgumentParser(description='Low-channel Batch 1: pool head only')
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print("  Low-channel Batch 1: pool head only")
    ...
    print("MODEL_TAG: lowchannel_b1_poolhead")
    print(
